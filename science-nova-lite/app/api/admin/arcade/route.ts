...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!topic_id || !subtype || !payload || !created_by) {
      return NextResponse.json(
        { error: 'Missing required fields: topic_id, subtype, payload, created_by' },
        { status: 400 }
      )
    }

    // Validate arcade subtypes
    const validSubtypes = ['QUIZ', 'FLASHCARDS', 'GAME']
    if (!validSubtypes.includes(subtype)) {
      return NextResponse.json(
        { error: `Invalid subtype ${subtype} for arcade content` },
        { status: 400 }
      )
    }
