        ...
        <Card className={`h-full backdrop-blur-lg ${theme.background.card} ${theme.border.secondary} border-2 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300`}>
          ...
          <CardContent>
            <div className="relative">
              <TransformWrapper initialScale={1} minScale={0.5} maxScale={3} wheel={{ step: 0.1 }} pinch={{ step: 5 }} doubleClick={{ mode: "reset" }}>
                <TransformComponent wrapperClass="!w-full !h-96 border-2 border-gray-300 rounded-xl overflow-hidden" contentClass="!w-full !h-full">
                  <div className="relative w-full h-full">
                    ...
                    {diagram.hotspots.map((hotspot, index) => (
                      <button key={index} className="absolute w-6 h-6 bg-blue-500 border-2 border-white rounded-full shadow-lg hover:bg-blue-600 transition-all duration-200 hover:scale-110 animate-pulse" style={{ left: `${hotspot.x}%`, top: `${hotspot.y}%`, transform: 'translate(-50%, -50%)' }} onClick={() => setSelectedHotspot(selectedHotspot === index ? null : index)}>
                        <span className="sr-only">{hotspot.label}</span>
                      </button>
                    ))}
                    {selectedHotspot !== null && diagram.hotspots[selectedHotspot] && (
                      <div className="absolute bg-black/80 text-white p-3 rounded-lg shadow-xl max-w-xs z-10" style={{ left: `${diagram.hotspots[selectedHotspot].x}%`, top: `${Math.max(diagram.hotspots[selectedHotspot].y - 10, 5)}%`, transform: 'translateX(-50%)' }}>
