...
interface Topic { id: string; title: string; grade_level: number; study_areas: { name: string; vanta_effect: string } | null }
...
export function TopicsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadingData(true)
      try {
        const { count: totalCount } = await supabase.from('topics').select('*', { count: 'exact', head: true })
        setTotalTopicsCount(totalCount || 0)

        let topicsQuery = supabase
          .from('topics')
          .select(`id, title, grade_level, study_area_id, study_areas!inner ( name, vanta_effect )`)
          .limit(6)
        ...
        const transformed = (topicsData || []).map((t: any) => ({ ...t, study_areas: t.study_areas?.[0] || null }))
        setRecommendedTopics(transformed)
      } finally {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <Button variant="ghost" className="mb-6" asChild>
            <Link href="/topics/all"><ArrowRight className="h-4 w-4 mr-2 rotate-180" /> Browse All Topics</Link>
          </Button>
          ...
          {recommendedTopics.length > 0 ? (
                ...
                <StaggerItem key={topic.id}>
                  <Link href={`/topic/${topic.id}`}>
                    <Card className="h-full group bg-white/80 backdrop-blur-md border-gray-300 border-2 hover:border-blue-400 hover:shadow-xl transform hover:scale-105 transition-all duration-300 cursor-pointer">
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-2">
                            <Badge className="bg-gradient-to-r from-purple-500 to-pink-500 text-white border border-purple-400">#{index + 1} Recommended</Badge>
                          </div>
                          <div className="flex flex-col gap-2">
                            ...
                            <Badge className="bg-gradient-to-r from-green-500 to-green-600 text-white border border-green-400">{topic.study_areas?.name || 'Science'}</Badge>
                          </div>
                        </div>
                      ...
                      <CardContent>
                        <CardDescription className="text-green-700"><span className="font-semibold text-purple-600">✨ AI Recommended: </span>This {topic.study_areas?.name?.toLowerCase() || 'science'} topic has been specially selected for grade {topic.grade_level} students. Content is pre-cached for instant access!</CardDescription>
                      </CardContent>
                    </Card>
                  </Link>
                </StaggerItem>
            ...
            <Card className="bg-white/80 backdrop-blur-md border-gray-300 border-2">
              <CardContent className="text-center py-8">
                ...
                <Button asChild><Link href="/topics/all"><BookOpen className="h-4 w-4 mr-2" /> Browse All Topics Instead</Link></Button>
              </CardContent>
            </Card>
          )}
